...
export class CheckoutService {
  ...
  async initiateRazorpayPayment(
    ...
    // Default for missing/unrecognized mode is "direct" — matches the
    // shopkeeper.schema default and the "every shop pastes their own keys"
    // policy. Legacy soft-onboarded shops still resolve to "route" because
    // they carry an explicit mode="standard" + an accountId.
    const explicitMode = shop.razorpay?.mode;
    const mode: "platform" | "direct" | "route" =
      explicitMode === "platform"
        ...
        : explicitMode === "route"
          ? "route"
          : explicitMode === "standard" && shop.razorpay?.accountId
            ? "route"
            : "direct";
