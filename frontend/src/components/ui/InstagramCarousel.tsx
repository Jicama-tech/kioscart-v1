...
export function InstagramCarousel({ urls, title, description, titleColor, descColor }: InstagramCarouselProps) {
  ...
  // Duplicate once for a seamless marquee (translateX -50%)
  const loopItems = [...validEmbeds, ...validEmbeds];
  const durationSec = Math.max(20, validEmbeds.length * 8);
  ...
  return (
    <section ref={containerRef} className="pt-8 sm:pt-12 lg:pt-16 pb-16 sm:pb-20 lg:pb-24 mb-8 sm:mb-12 overflow-hidden">
      ...
      <div className="relative overflow-hidden">
        <div
          className="flex gap-4 sm:gap-6 px-4 animate-ig-marquee hover:[animation-play-state:paused]"
        ...
        >
          {loopItems.map((item, i) => (
            <a
              ...
              className="flex-shrink-0 block rounded-2xl overflow-hidden bg-white shadow-md relative"
