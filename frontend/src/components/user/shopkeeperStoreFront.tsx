...
import { useState, useEffect, CSSProperties, useMemo, useCallback, useRef, lazy, Suspense } from "react";
...
// Lazy load heavy components — not needed on initial storefront render
const StorefrontCustomizer = lazy(() => import("@/components/shopkeeper/StorefrontCustomizer").then(m => ({ default: m.StorefrontCustomizer })));
const ProductDetailsDialog = lazy(() => import("@/components/shopkeeper/productDetailDialog").then(m => ({ default: m.ProductDetailsDialog })));
import {
  FaFacebook,
  FaInstagram,
  FaTiktok,
...
export function StorefrontTemplate({ onBack }: { onBack: () => void }) {
  ...
  const { formatPrice, getSymbol } = useCurrency(country || "IN");
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const shopkeeperId =
          bundle.store.shopkeeperId?.toString() || "";
        if (!shopkeeperId)
          throw new Error("No shopkeeperId in storefront data");

        setShopkeeperId(shopkeeperId);

        if (bundle.shopkeeper) {
          setWhatsappNumber(bundle.shopkeeper.whatsappNumber || "");
          setShopkeeperInfo(bundle.shopkeeper);
          sessionStorage.setItem("country", bundle.shopkeeper.country);
          setCountry(bundle.shopkeeper.country);
        }
  ...
  // Memoize carousel products — only recalculate when products change
  const carouselProducts = useMemo(() => {
    if (!products || products.length === 0) return [];
    const shuffled = [...products];
    // Fisher-Yates shuffle (proper O(n) shuffle)
  ...
  const themeColors = useMemo(() => {
    const theme = settings?.settings?.design?.theme;
    ...
    return {
      ...
      "--primary": settings?.settings?.design?.primaryColor || "#6366f1",
      "--secondary": settings?.settings?.design?.secondaryColor || "#8b5cf6",
    };
  }, [settings?.settings?.design?.theme, settings?.settings?.design?.primaryColor, settings?.settings?.design?.secondaryColor]);

  const themeStyles = useMemo<CSSProperties>(() => ({
    ...themeColors,
    fontFamily: settings?.settings?.design?.fontFamily || "inherit",
  }) as CSSProperties, [themeColors, settings?.settings?.design?.fontFamily]);
  ...
  // Enhanced pricing logic to handle both product-level and variant-level pricing
  ...
  // Helper function for displaying price
  function getDisplayPrice(product: any) {
    ...
    //     {/* Crossed original price only for non-variant discounted products */}
    //     {isDiscounted && !hasVariants && originalPrice && (
    //       <span className="text-sm text-muted-foreground line-through">
    //         {formatPrice(originalPrice)} / {product.measurement}
    //       </span>
    //     )}
    ...
    //     {/* Variants: just show discount info */}
    //     {hasVariants && isDiscounted && (
  ...
  // themeStyles moved before early returns — see above
  ...
  return (
    <div className="storefront-light min-h-screen bg-background">
      ...
      {/* Product Details Dialog — lazy loaded, only renders when open */}
      {showProductDialog && (
            ...
            >
              ...
              <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[0.8fr_1.2fr] gap-4 sm:gap-8 items-center relative z-10">
                ...
                <div className="flex justify-center md:justify-end w-full order-1 lg:order-2">
                  <div className="w-full max-w-[16rem] md:max-w-[14rem] lg:max-w-[32rem] h-56 sm:h-64 md:h-72 lg:h-[24rem] bg-card/10 rounded-3xl overflow-hidden shadow-2xl relative">
                    {design.bannerImage ? (
                      <img
                        loading="lazy"
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center">
                  ...
                  <div className="space-y-4 sm:space-y-6 order-2 lg:order-2">
                    ...
                    <p className="text-sm sm:text-base lg:text-lg text-muted-foreground leading-relaxed line-clamp-2">
                        ...
                        */}
                              {carouselProducts
                                ...
                                .map((product, index) => (
                                  <Card
                                    key={product._id}
                                    className={`group cursor-pointer hover:shadow-xl transition-all duration-300 rounded-3xl sm:rounded-3xl overflow-hidden bg-card h-full relative ${
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 lg:gap-8">
                {filteredProducts.map((product) => (
                  <div
                    key={product._id}
                    className="group cursor-pointer border border-border/50 hover:border-primary/60 rounded-2xl overflow-hidden bg-card hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    onClick={() => handleProductClick(product._id)}
                  >
              ...
              {filteredProducts.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                  {filteredProducts.map((product) => (
                    <Card
                      key={product._id}
                      className="group cursor-pointer hover:shadow-2xl transition-all duration-300 rounded-2xl overflow-hidden bg-card border-0 shadow-md"
                      onClick={() => handleProductClick(product._id)}
                    >
                      <div className="relative aspect-square">
                        ...
                        {product.category && (
                          <div className="absolute top-3 right-3">
                            <span className="text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full bg-card/90 backdrop-blur-sm text-foreground shadow-sm">
                              {product.category}
                            </span>
                          </div>
                        )}
                  ...
                  {features.showSocialMedia && (
                    <div className="flex flex-col items-center lg:hidden w-full">
                      ...
                      <div className="flex items-center justify-center gap-3">
                        ...
                        {!["IN", "IND", "INDIA"].includes((country || "").toString().trim().toUpperCase()) &&
                ...
                <div className="flex items-center space-x-4 mt-2 md:mt-0">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <button className="text-foreground font-semibold hover:underline cursor-pointer">
                        Terms & Conditions
                      </button>
                    </DialogTrigger>
                    <DialogContent className="max-w-2xl max-h-[85vh] p-0 overflow-hidden border-none shadow-2xl">
                      {/* Header with a subtle gradient background */}
                        ...
                        <DialogHeader>
                          <DialogTitle className="text-2xl font-extrabold tracking-tight text-foreground">
                            ...
                            <span className="block text-sm font-normal text-muted-foreground mt-1">
                              Legal agreement for {general.storeName}
                            </span>
                          </DialogTitle>
                        </DialogHeader>
                      </div>

                      {/* Content Area with custom scrollbar styling */}
                      <div className="px-8 mb-10 overflow-y-auto max-h-[60vh] scrollbar-thin">
                        <div
      ...
      quill-content"
                          dangerouslySetInnerHTML={{
                            __html:
                              shopkeeperInfo.termsAndConditions ||
                              "No terms provided.",
                          }}
                        />
                      </div>
          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="border-t border-gray-800 pt-6 sm:pt-4 mt-4">
                ...
                <div className="flex items-center justify-center space-x-6 mt-4 w-full">
                  {/* --- TERMS AND CONDITIONS DIALOG --- */}
                  <Dialog>
                    <DialogTrigger asChild>
                      <button className="text-muted-foreground font-medium hover:text-foreground transition-colors cursor-pointer outline-none">
                        Terms & Conditions
                      </button>
                    </DialogTrigger>
                    <DialogContent className="max-w-2xl max-h-[85vh] p-0 overflow-hidden border-none shadow-2xl">
      ...
      quill-content"
                  ...
                  <a
                    href="/login"
                    className="text-muted-foreground font-medium hover:text-foreground transition-colors no-underline"
                  >
                    Store login
                  </a>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-base sm:text-lg">
              ...
              <div className="border-t border-border/70 pt-7 sm:pt-9 pb-4 sm:pb-6 flex flex-col md:flex-row items-center justify-between text-sm sm:text-base text-muted-foreground">
                ...
                <div className="flex items-center space-x-4 mt-2 md:mt-0">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <button className="text-foreground font-semibold hover:underline mt-2 md:mt-0">
                        Terms & Conditions
                      </button>
                    </DialogTrigger>
