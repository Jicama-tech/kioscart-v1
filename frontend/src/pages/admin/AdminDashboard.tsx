...
export function AdminDashboard({ onLogout }: AdminDashboardProps) {
      ...
      if (resData.pendingApprovals) {
        ...
        setPendingApprovals([
          ...organizers.map((o: any) => ({
            ...o, id: o._id, name: o.name, type: "Organizer", email: o.email,
            appliedDate: o.createdAt ? new Date(o.createdAt).toLocaleDateString() : "N/A",
          })),
          ...shopkeepers.map((s: any) => ({
            ...s, id: s._id, name: s.name, type: "Shopkeeper", email: s.email,
            referredBy: s.referredBy || null,
            appliedDate: s.createdAt ? new Date(s.createdAt).toLocaleDateString() : "N/A",
          })),
        ]);
      }

      if (resData.recentActivity) {
        setRecentActivity(resData.recentActivity.map((act: any) => ({
          ...act,
          time: typeof act.time === "string" ? new Date(act.time).toLocaleString() : act.time,
        })));
      }
    } catch (err: any) {
  ...
  const handleApprove = async () => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      const response = await fetchWithLoading(`${apiURL}/admin/approve/${selectedApplicant.id}`, {
        method: "PATCH",
  ...
  const handleReject = async () => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      const response = await fetchWithLoading(`${apiURL}/admin/reject/${selectedApplicant.id}`, {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ role: selectedApplicant.type }),
      });
      ...
      <header className="h-14 sm:h-16 border-b bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 flex items-center px-3 sm:px-6 sticky top-0 z-50">
        <div className="flex items-center flex-1">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-sm sm:text-lg font-bold text-slate-900">KiosCart Admin</h1>
              <p className="text-[10px] sm:text-xs text-muted-foreground hidden sm:block">Management Console</p>
            </div>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          <Button variant="outline" size="sm" onClick={() => setCreateDialogOpen(true)} className="text-xs sm:text-sm">
              ...
              <TabsContent value="dashboard" className="mt-0">
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
                    {[
                      { title: "Total Users", value: stats.totalUsers, icon: Users, color: "text-blue-600", iconColor: "text-blue-500", note: "Registered users" },
                      { title: "Total Events", value: stats.totalEvents, icon: Calendar, color: "text-purple-600", iconColor: "text-purple-500", note: `${stats.thisMonthEvents} this month` },
                      { title: "Active Organizers", value: stats.activeOrganizers, icon: TrendingUp, color: "text-green-600", iconColor: "text-green-500", note: "Approved organizers" },
                      { title: "Active Shopkeepers", value: stats.activeShopkeepers, icon: Store, color: "text-orange-600", iconColor: "text-orange-500", note: "Approved stores" },
                    ].map((stat, i) => (
                      <Card key={i}>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                          <CardTitle className="text-xs sm:text-sm font-medium truncate">{stat.title}</CardTitle>
                          <stat.icon className={`h-4 w-4 ${stat.iconColor} flex-shrink-0`} />
                        </CardHeader>
                        <CardContent className="pt-0">
                          <div className={`text-lg sm:text-2xl font-bold ${stat.color}`}>{stat.value}</div>
                          <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">{stat.note}</p>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                      ...
                      <CardHeader className="pb-3">
                        <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
                          ...
                          {stats.pendingApprovals > 0 && (
                            <Badge variant="destructive" className="text-xs">{stats.pendingApprovals}</Badge>
                          )}
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                              ...
                              <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/30 transition-colors">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="text-xs text-muted-foreground truncate">
                                    ...
                                    {item.referredBy && ` • Ref: ${item.referredBy.name}`}
                                  </p>
                                  <p className="text-[10px] text-muted-foreground">Applied: {item.appliedDate}</p>
                                </div>
                                <Button size="sm" variant="outline" className="ml-2 text-xs shrink-0" onClick={() => handleReviewClick(item)}>
                                  Review
                                </Button>
                              </div>
                            ))}
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
                          <Clock className="h-4 w-4 sm:h-5 sm:w-5 text-blue-500" /> Recent Activity
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        {recentActivity.length === 0 ? (
                          ...
                          <div className="space-y-3 max-h-[350px] overflow-y-auto">
                            {recentActivity.map((act) => (
                              <div key={act.id} className="flex items-center gap-3 p-2.5 border rounded-lg hover:bg-muted/30 transition-colors">
                                <div className={`p-1.5 rounded-full shrink-0 ${
                                  act.status === "pending" ? "bg-yellow-100 text-yellow-600"
                                    : act.status === "approved" ? "bg-green-100 text-green-600"
                                    : "bg-blue-100 text-blue-600"
                                }`}>
                                  {act.status === "pending" ? <Clock className="h-3.5 w-3.5" />
                                    : act.status === "approved" ? <CheckCircle className="h-3.5 w-3.5" />
                                ...
                                <Badge variant={act.status === "pending" ? "secondary" : act.status === "approved" ? "default" : "outline"} className="text-xs shrink-0 capitalize">
                                  {act.status}
                                </Badge>
                              </div>
                            ))}
      ...
      <Dialog open={reviewDialogOpen} onOpenChange={setReviewDialogOpen}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Review and approve or reject this application</DialogDescription>
          </DialogHeader>
          {selectedApplicant && (
            <div className="space-y-4 mt-2">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: "Business Email", value: selectedApplicant.businessEmail },
                  ...
                  { label: "Referred By", value: selectedApplicant.referredBy?.name ? `${selectedApplicant.referredBy.name} (${selectedApplicant.referredBy.referralCode})` : null },
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="space-y-1">
              <Label>Email</Label>
              <Input type="email" value={newAdmin.email} onChange={(e) => setNewAdmin({ ...newAdmin, email: e.target.value })} placeholder="admin@email.com" />
            </div>
            <div className="space-y-1">
              <Label>Password</Label>
              <div className="relative">
                <Input type={showPassword ? "text" : "password"} value={newAdmin.password} onChange={(e) => setNewAdmin({ ...newAdmin, password: e.target.value })} placeholder="Password" />
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                <Input type={showConfirmPassword ? "text" : "password"} value={newAdmin.confirmPassword} onChange={(e) => setNewAdmin({ ...newAdmin, confirmPassword: e.target.value })} placeholder="Confirm password" />
                <Button type="button" variant="ghost" size="sm" className="absolute right-1 top-1 h-7 w-7 p-0" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>
                  {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </Button>
              </div>
            </div>
          </div>
          <Button className="w-full mt-4" onClick={handleCreateAdmin}>Create Admin</Button>
        </DialogContent>
      </Dialog>
    </div>
  );
